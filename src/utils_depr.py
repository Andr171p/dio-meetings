...
def split_audio_into_chunks(
    ...
    for i, chunk in enumerate(chunks):
        file_path = output_dir / f"chunk_{i}.{output_format}"
        chunk.export(file_path, format=output_format, bitrate="256k")
        logger.info(
            "Export `%s` chunk to %s format", file_path, output_format.upper()
        )
